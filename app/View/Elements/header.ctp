	...
	<div id="headerContent" class="plot cf">
		...
		<nav id="hNav">
			<ul class="cf">
				...
				<a href="<?php echo ROOT_URL . 'Accounts/profile?id=' . $this->Session->read('Auth.id'); ?>"><?php echo $this->Html->image(IMG_PATH . $this->Session->check('Auth.profImg') ? $this->Session->read('Auth.profImg') : NO_IMG_URL, array("alt" => "マイプロフィール")); ?><span id="name"><?php echo ($this->Session->read('Auth.last_name') == null ? '':h($this->Session->read('Auth.last_name'))) . ' ' . ($this->Session->read('Auth.first_name') == null ? '':h($this->Session->read('Auth.first_name'))); ?></span></a></li>
